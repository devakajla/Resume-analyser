...
type Job = { id: string; title: string; company: string; applicant_count: number };
...
function ApplicantsIndex() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-3xl font-semibold text-foreground">Applicants</h1>
        <p className="mt-2 text-sm text-muted-foreground">Pick a role to inspect candidate scorecards.</p>
      </div>
            ...
            {jobs.map((j) => (
              <li key={j.id}>
                ...
                >
                  ...
                  <div className="text-sm text-muted-foreground">{j.applicant_count} applicants</div>
                </Link>
              </li>
            ))}
          </ul>
